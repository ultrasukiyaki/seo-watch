...
- スーパーユーザーのメール追加・変更と、閲覧ユーザー招待のメール重複確認を修正するコードのみの更新です。
- DBマイグレーションおよび既存データの変更はありません。
- v0.11.0でメール追加時に`SQLSTATE[HY093]`が発生した場合は、更新後にアカウント画面からもう一度保存してください。

## v0.11.1からv0.12.0

- 通常どおりファイルを更新すると、初回アクセスまたはCLI起動時に変動通知用テーブルと標準ルールが自動作成されます。手動SQLは不要です。
- 既存のSearch Consoleデータ、ユーザー、メール設定、改善タスクは維持されます。
- Cronへ`detect-alerts.php`と`send-alert-digest.php`を同期後の時刻に追加してください。
- メール通知は初期状態で無効です。各ユーザーが確認済みメールを用意し、アカウント画面から有効化します。
- 配送無効時は確認待ちとして保存されます。配送設定後、確認メールの送信・確認完了まで実施してください。

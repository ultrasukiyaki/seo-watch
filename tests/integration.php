...
$assert((bool)$pdo->query("SHOW TABLES LIKE 'alert_rules'")->fetchColumn(), 'alert rules migration');
$assert((bool)$pdo->query("SHOW TABLES LIKE 'alert_occurrences'")->fetchColumn(), 'alert occurrences migration');
$assert((bool)$pdo->query("SHOW TABLES LIKE 'alert_deliveries'")->fetchColumn(), 'alert deliveries migration');
$assert((int)$pdo->query('SELECT COUNT(*) FROM alert_rules')->fetchColumn() === 10, 'ten default alert rules');
$assert((int)$pdo->query('SELECT COUNT(*) FROM alert_rules WHERE enabled=0')->fetchColumn() === 1, 'email-independent low CTR default disabled');
$alertIndexes = $pdo->query("SHOW INDEX FROM alert_occurrences WHERE Key_name='uq_alert_occurrence_period'")->fetchAll();
$assert(count($alertIndexes) === 3, 'alert occurrence idempotency index');
